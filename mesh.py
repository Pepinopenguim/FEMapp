...
class MeshEngine:
    def __init__(self, model: FEMModel, math_helper: CurveHelper):
        """
    ...
    def generate_mesh(self, mesh_size: float = 5.0) -> Tuple[List[Tuple[float, float]], List[Tuple[int, int, int]]]:
        ...
        # ---> RENAMED: Iterating over edges instead of elements
        ...
        # Extract Triangles (Element Type 2 in Gmsh is a 3-node triangle)
    ...
    def generate_mesh(self, mesh_size: float = 5.0) -> Tuple[List[Tuple[float, float]], List[Tuple[int, int, int]]]:
        ...
        for eid, elem in self.model.edges.items():
            ...
            if elem.type == "linear":
                ...
                n_mid = self.model.nodes[elem.mid_node]
